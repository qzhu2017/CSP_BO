...
//	int m,n,d,x2i;
    ...
    int *x1_indices,*ele1,*x1_inds;
    double *dx1dr,*x1,*Cbig,*d2k_dx1dx2,*ans;
				...
				if(_ele1==_ele2 && x2_norm > eps){
				    ...
				    for(i=0;i<d;i++){
				    	for(j=0;j<d;j++){
				    	   dval=0;
				           if(i==j) dval=1.0;
				           d2d_dx1dx2=(dval-_x2[i]*_x2[j]/x2_norm2)/x1x2_norm+
				        		   (_x1[i]*_x2[j]*x1x2_dot/x2_norm2-_x1[i]*_x1[j])/x1_norm3/x2_norm;

				           d2k_dx1dx2[i*d+j]=(((_x2[i]/x1x2_norm-_x1[i]*x1x2_dot/x1_norm3/x2_norm)*
				        		               (_x1[j]/x1x2_norm-_x2[j]*x1x2_dot/x1_norm/x2_norm3) )*d2*(zeta-1) +
...
int main() {
	...
	kff_many(1,2,3,4,ptemp1,ptemp2);

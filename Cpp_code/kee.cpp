...
void kee_single(double* x1, double* x2, int* x2_indices, double sigma, double l, double zeta, int M, int N, int P, double eps){
    // M: length of x1; 
    // N: length of x2; 
    ...
    for(unsigned int i=0;i<M;i++){
        for(unsigned int j=0;j<N;j++){
            ...
            k[i*N+j] = sigma2 * exp((1-D[i*N+j])/(2*l2));
        }
    }
...
int main() {
    ...
    const auto M = 5, N = 20, D = 5;
    int x2_indices[5] = {2, 4, 6, 3, 5};
    int len = sizeof(x2_indices)/sizeof(x2_indices[0]);
    ...
    double sigma=28.835, l=0.01, zeta=2.0, eps=1e-8;

    //double *x1, double *x2, int *x2_indices, double sigma, double l, double zeta, int M, int N, int P, double eps

    high_resolution_clock::time_point t1 = high_resolution_clock::now();
    kee_single(x1, x2, x2_indices, sigma, l, zeta, M, N, D, eps);

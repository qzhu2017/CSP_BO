...
///////////////kff///////////
        x1_norm= new double[m];
        x1_norm2= new double[m];
        x1_norm3= new double[m];
		for(i=0;i<m;i++){
			dval=0.0;
			for(j=0;j<d;j++){
		...
		x1_x1_norm3=new double[m*d];
		for(i=0;i<m;i++){
			for(j=0;j<d;j++){
				x1_x1_norm3[i*d+j]=x1[i*d+j]/x1_norm3[i];
			}
		}
		dd_dx1=new double[m*n*d];
		dd_dx2=new double[m*n*d];
		for(i=0;i<m;i++){
			for(j=0;j<n;j++){
				for(k=0;k<d;k++){
					dd_dx2[(i*n+j)*d+k]=
		...
		d2d_dx1dx2=new double[m*n*d*d];
		d2k_dx1dx2=new double[m*n*d*d];
		for(i=0;i<m;i++){
			for(j=0;j<n;j++){
				for(k=0;k<d;k++){
					for(l=0;l<d;l++){

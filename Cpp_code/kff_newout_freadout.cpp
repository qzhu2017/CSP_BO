...
void kff_many(){
	...
	int m,n,d,x2i,m1;
    int *mi,*ele_all,*x2_indices,*x2_inds,*ele1;
    ...
    int totalsize,totalm=0;
    int mempoint,memsize;
    for (ii=0;ii<m1;ii++) totalm+=mi[ii];
    totalsize=totalm*d*sized+totalm*d*3*sized+totalm*sizei;
    char *mdata=new char[totalsize];
    fread(mdata, sizec, totalsize, filp);
    mempoint=0;
...
//		fread(ele1, sizei, m, filp);
	    ...
	    memsize=mi[ii]*d*sized;
	    memcpy(x1,mdata+mempoint,memsize);
	    mempoint+=memsize;
	    memsize=mi[ii]*d*3*sized;
	    memcpy(dx1dr,mdata+mempoint,memsize);
	    mempoint+=memsize;
	    memsize=mi[ii]*sizei;
		memcpy(ele1,mdata+mempoint,memsize);
		mempoint+=memsize;

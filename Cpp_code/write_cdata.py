...
#    print(struct.pack('i',d)) 
#    print(struct.pack('d',tempd))   
#    exit()
    
    file=open('c.dat',"wb")
#    binary_format = bytearray(n)
    file.write(struct.pack('i',m2p)) #n
#    binary_format = bytearray(n)
    d=len(x_all[0])
    file.write(struct.pack('i',d)) #d
    file.write(struct.pack('i',m2)) #x2i
    file.write(struct.pack('i',m1)) #m1
    
    for i in range(m1):
        (x1, dx1dr, ele1) = X1[i]
#        print(i,len(x1))
        file.write(struct.pack('i',len(x1))) #mi[m1]
#    file.write(struct.pack('d',m1))
    for i in range(m2p):
        for j in range(d):
            file.write(struct.pack('d',x_all[i][j]))
    for i in range(m2p):
        for j in range(d):
            for k in range(3):
              file.write(struct.pack('d',dxdr_all[i][j][k]))
    for i in range(m2p):
        file.write(struct.pack('i',ele_all[i]))
    for i in range(m2):
        file.write(struct.pack('i',x2_indices[i]))
        
    for i in range(m1):
        (x1, dx1dr, ele1) = X1[i]
        for j in range(len(x1)):
            for k in range(d):
                file.write(struct.pack('d',x1[j][k]))      
        for j in range(len(x1)):
            for k in range(d):
                for l in range(3):
                   file.write(struct.pack('d',dx1dr[j][k][l])) 
        for j in range(len(x1)):
            file.write(struct.pack('i',ele1[j]))     
    ...
    for i in range(m1):
        ...
        if len(ids[0]) > 0:
            dk_dD[ids] = 0
#        print(i,'x1',len(x1),len(x1[0]))
        print(i,'x_all',len(x_all),len(x_all[0]))
#        print(i,'dx1dr',len(dx1dr),len(dx1dr[0]),len(dx1dr[0][0]))
#        print(i,'ele1',len(ele1))
        print(i,'dxdr_all',len(dxdr_all),len(dxdr_all[0]),len(dxdr_all[0][0]))
        print(i,'ele_all',len(ele_all))
        print(i,'x2_indices',len(x2_indices))
#         print(i,'dk_dD',len(dk_dD),len(dk_dD[0]))

...
class SO3:
    ...
    def calculate(self, atoms, atom_ids=None):
        ...
        ctest = np.zeros(1, dtype=np.complex128)
        ctest[0]=1.2+3.4j
#        print(ctest)
#        exit()
     
        pdat_ctest=ffi.new('double _Complex['+str(1)+']',list(ctest.ravel()))
        knatom=8
        lcen1=len(self.center_atoms)
        lcen2=len(self.center_atoms[0])
        pdat_center_atoms=ffi.new('double['+str(lcen1*lcen2)+']',list(self.center_atoms.ravel()))
#        print("center atoms len1, len2",lcen1,lcen2)
        
        lne1=len(self.neighborlist)
        lne2=len(self.neighborlist[0])
        lne3=len(self.neighborlist[0][0])
        pdat_neighborlist=ffi.new('double['+str(lne1*lne2*lne3)+']',list(self.neighborlist.ravel()))
#        print('seq',self.seq)
#        exit()
        lseq1=len(self.seq)
        lseq2=len(self.seq[0])
        pdat_seq=ffi.new('int['+str(lseq1*lseq2)+']',list(self.seq.ravel()))    
        
        lans1=len(self.atomic_numbers)
        lans2=len(self.atomic_numbers[0])
        pdat_atomic_numbers=ffi.new('int['+str(lans1*lans2)+']',list(self.atomic_numbers.ravel()))               
        
        lpli1=len(self._plist)
        lpli2=len(self._plist[0])
        plist_r = np.zeros((lpli1,lpli2), dtype=np.float64)
        plist_i = np.zeros((lpli1,lpli2), dtype=np.float64)
        plist_r[:][:]=self._plist[:][:].real
        plist_i[:][:]=self._plist[:][:].imag       
        pdat_plist_r=ffi.new('double['+str(lpli1*lpli2)+']',list(plist_r.ravel()))               
        pdat_plist_i=ffi.new('double['+str(lpli1*lpli2)+']',list(plist_i.ravel()))  
        
        ldpl1=len(self._dplist)
        ldpl2=len(self._dplist[0])
        ldpl3=len(self._dplist[0][0])
        dplist_r = np.zeros((ldpl1,ldpl2,ldpl3), dtype=np.float64)
        dplist_i = np.zeros((ldpl1,ldpl2,ldpl3), dtype=np.float64)
        plist_r[:][:][:]=self._plist[:][:][:].real
        plist_i[:][:][:]=self._plist[:][:][:].imag       
        pdat_dplist_r=ffi.new('double['+str(ldpl1*ldpl2*ldpl3)+']',list(dplist_r.ravel()))               
        pdat_dplist_i=ffi.new('double['+str(ldpl1*ldpl2*ldpl3)+']',list(dplist_i.ravel()))  
        
        lpst1=len(self._pstress)
        lpst2=len(self._pstress[0])
        lpst3=len(self._pstress[0][0])
        lpst4=len(self._pstress[0][0][0])
        pstress_r = np.zeros((lpst1,lpst2,lpst3,lpst4), dtype=np.float64)
        pstress_i = np.zeros((lpst1,lpst2,lpst3,lpst4), dtype=np.float64)
        pstress_r[:][:][:][:]=self._pstress[:][:][:][:].real
        pstress_i[:][:][:][:]=self._pstress[:][:][:][:].imag       
        pdat_pstress_r=ffi.new('double['+str(lpst1*lpst2*lpst3*lpst4)+']',list(pstress_r.ravel()))               
        pdat_pstress_i=ffi.new('double['+str(lpst1*lpst2*lpst3*lpst4)+']',list(pstress_i.ravel()))                
        ...
        if self.stress == True:
            ...
            istre=0
#        lib.stest(pdat_center_atoms)
#        exit()
                        
        lib.spectrum(lcen1,lcen2,pdat_center_atoms,lne1,lne2,lne3,pdat_neighborlist,lseq1,lseq2,pdat_seq,lans1,lans2,pdat_atomic_numbers,self.nmax, self.lmax, self.rcut, self.alpha, ider, istre,lpli1,lpli2,pdat_plist_r,pdat_plist_i,ldpl1,ldpl2,ldpl3,pdat_dplist_r,pdat_dplist_i,lpst1,lpst2,lpst3,lpst4,pdat_pstress_r,pdat_pstress_i)
#        exit()
...
#                                      self.stress, self._plist, self._dplist, self._pstress)
        ...
        plist_r = np.frombuffer(ffi.buffer(pdat_plist_r, lpli1*lpli2*8), dtype=np.float64)
        plist_r.shape = (lpli1, lpli2)
        self._plist[:][:].real=plist_r[:][:]
...
if  __name__ == "__main__":
    ...
    parser.add_option("-l", "--lmax", dest="lmax", default=3, type=int,
                      help="lmax, default: 3"
                      )

    parser.add_option("-n", "--nmax", dest="nmax", default=3, type=int,
                      help="nmax, default: 3"
                      )
    ...
    parser.add_option("-s", dest="stress", default=False, 
                      action='store_true',help='derivative flag')

    parser.add_option("-f", dest="der", default=False,
...
#    print(lmax,nmax,rcut,alpha,der,stress)
#    exit()

...
#        print("N_max,totalrow",N_max,totalrows)
#        sys.stdout.flush()

        ish=int(totalrows/nprocs)
        irest=totalrows-ish*nprocs
...
#        print("n_mpi_pr,ndim_pr",n_mpi_pr,ndim_pr)
#        print("max ndim,max_rank",max_ndim_pr,max_ranks)
#        sys.stdout.flush()
        rowid=0
        pts_to_add = {"energy": [], "force": [], "db": []}

        for mi in range(max_ndim_pr):

            select_id=mi+n_mpi_pr[rank]
#            print('rank, select id',rank,select_id)
            if ndim_pr[rank] > mi:
                rowid=int(select_id)
#                row=db.get(id=rowid)
                row=rows[rowid]
...
#                print("rank,row energy",rank,row.data.energy)
                # substract the energy/force offsets due to the base_potential
                if self.base_potential is not None:
...
#                print('rank,mi,energy_in,force_in',rank,mi,energy_in,force_in)
#                sys.stdout.flush()
                comm.Barrier()
                ...
                for mj in range(max_ranks):
                     select_rank=mj
                     if ndim_pr[select_rank] > mi :
#                        print('rank,mi,mj,ndim_[s_r],select id,rank',rank,mi,mj,ndim_pr[select_rank],select_id,select_rank)
#                        sys.stdout.flush()
#                        comm.barrier()
                        Revenergy_in=comm.bcast(energy_in, root=select_rank)
                        Revenergy=comm.bcast(energy, root=select_rank)
                        Revd=comm.bcast(d, root=select_rank)
                        Revatoms=comm.bcast(atoms, root=select_rank)
                        Revele=comm.bcast(ele, root=select_rank)
                        comm.Barrier()
                        if Revenergy_in:
                            pts_to_add["energy"].append((Revd['x'], Revenergy/len(Revatoms), Revele))
#                            print("pts_to_add[energy][0]", pts_to_add["energy"][count-1])
                        comm.Barrier()   
                        Revforce_in=comm.bcast(force_in, root=select_rank)
#                        print('rank,mj,Revenergy_in,Revforce_in',rank,mj,Revenergy_in,Revforce_in)
                        Revforce=comm.bcast(force, root=select_rank)
                        comm.Barrier()
                        for id in Revforce_in:
                            ids = np.argwhere(Revd['seq'][:,1]==id).flatten() 
                            _i = Revd['seq'][ids, 0]
                            pts_to_add["force"].append((Revd['x'][_i,:], Revd['dxdr'][ids], Revforce[id], Revele[_i]))

                
                        pts_to_add["db"].append((Revatoms, Revenergy, Revforce, Revenergy_in, Revforce_in))
                        comm.Barrier()
        comm.Barrier()
#        print("rank,atoms1,atoms2",rank,pts_to_add["db"][0][0],pts_to_add["db"][1][0])

#        print("rank,energy1,energy2",rank,pts_to_add["db"][0][1],pts_to_add["db"][1][1])
#        print("rank,force1,force2",rank,pts_to_add["db"][0][2],pts_to_add["db"][1][2])
#        print("rank,energy_in1,energy_in2",rank,pts_to_add["db"][0][3],pts_to_add["db"][1][3])        
#        print("rank,force_in1,force_in2,force_in3",rank,pts_to_add["db"][0][4],pts_to_add["db"][1][4],pts_to_add["db"][2][4])
